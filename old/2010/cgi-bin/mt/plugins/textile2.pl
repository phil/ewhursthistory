...
# ---------------------------------------------------------------------------
# $Id: textile2.pl,v 1.10 2004/02/17 06:37:25 brad Exp $
# ---------------------------------------------------------------------------

package plugins::textile2;

use strict;

use vars qw($VERSION $_initialized $Have_SmartyPants);
$VERSION = 2.0;
...
MT->add_text_filter('textile_2' => {
    label => 'Textile 2',
    on_format => \&textile_2,
    docs => 'http://www.bradchoate.com/mt/docs/mtmanual_textile2.html'
});

MT::Template::Context->add_container_tag(Textile => \&Textile);
MT::Template::Context->add_tag(TextileOptions => \&TextileOptions);
MT::Template::Context->add_tag(TextileHeadOffset => \&TextileHeadOffset);
...
sub Textile {
    ...
    my $str = MT::Template::Context::_hdlr_pass_tokens(@_);
    textile_2($str, $ctx);
}
