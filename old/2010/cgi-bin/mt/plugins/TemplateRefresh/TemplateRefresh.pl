...
sub BEGIN {
    MT->add_plugin(new MT::Plugin::TemplateRefresh({
        ...
        description => "Backup and refresh existing templates to Movable Type's default templates.",
        version => 1.0,
    }));
}
...
sub default_templates {
    ...
    return $app->error("Error loading default templates.") unless $tmpl_list;
    $tmpl_list;
}
    ...
    foreach my $blog_id (@id) {
        ...
        if (!$app->{author}->is_superuser()) {
            ...
            if (!$perms || (!$perms->can_edit_templates() && !$perms->can_administer_blog())) {
                push @msg, "Insufficient permissions to modify templates for weblog '" . $blog->name() . "'";
                next;
            }
        }

        push @msg, "Processing templates for weblog '" . $blog->name . "'";
        ...
        foreach my $val (@$tmpl_list) {
            ...
            if ($tmpl) {
                ...
                if (!$dict->{$val->{type}}{$orig_name}{$digest}) {
                    ...
                    $backup->save;
                    push @msg, "Refreshing (with <a href=\"?__mode=view&amp;blog_id=$blog_id&amp;_type=template&amp;id=" . $backup->id . "\">backup</a>) template '" . $tmpl->name . "'.";
                } else {
                    push @msg,'Refreshing template \'' . $tmpl->name . '\'.';
                }
                ...
                push @msg, 'Created template \'' . $tmpl->name . '\'.';
            }
        }
    }
    my @msg_loop;
...
sub refresh_individual_templates {
    ...
    foreach my $tmpl_id (@id) {
        ...
        if (!$val) {
            push @msg, "Skipping template '" . $tmpl->name . "' since it appears to be a custom template.";
            next;
        }
