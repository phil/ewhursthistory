...
#
# $Id: Comments.pm 16342 2005-08-17 21:50:11Z ezra $
...
sub do_red {
    ...
    return $app->error("No such comment")
...
sub post {
    ...
    $comment->save or MT::log("Comment save failed with " .
                              $comment->errstr);
    if ($commenter) {
        $commenter->url($comment->url) if $comment->url;
        $commenter->save or MT::log("Commenter save failed with " .
                                    $commenter->errstr);
    }
...
sub _send_comment_notification {
    ...
    my $commenter_has_comment = _commenter_has_comment($commenter, $entry);
    my $attn_reqd = $commenter ?
          (!$commenter_has_comment && $comment->is_moderated())
        : $comment->is_moderated();

    unless ($blog->email_all_comments ||
        ($attn_reqd && $blog->email_attn_reqd_comments)) {
        return;
    }
...
#    my ($r, $s) = split /:/, decode_url($sig_str);
    ...
    MT::log("The TypeKey signature is out of date (" . ($params{ts} - time)
            . " seconds old). Ensure that your server's clock is correct\n")
    ...
    if ($q->param('logout')) {
        ...
        if ($sig_str) {
                                           ...
                                           ts => $ts))
            {
                ...
                return $app->error("The sign-in validation failed.");
            }
