...
#
# $Id: Search.pm 17280 2005-09-08 21:42:02Z bchoate $
...
sub _straight_search {
    ...
    $app->log($app->translate("Search: query for '[_1]'",
              $app->{search_string}));
...
sub _hdlr_entry_edit_link {   
    ...
    my $app = MT->instance;
    sprintf
      q([<a href="%s%s%s">Edit</a>]),
      $url, $cfg->AdminScript, $app->uri_params('mode' => 'view', args => { '_type' => 'entry', id => $entry->id, blog_id => $blog_id});
}
