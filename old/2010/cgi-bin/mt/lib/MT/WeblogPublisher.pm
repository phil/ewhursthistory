...
#
# $Id: WeblogPublisher.pm 16232 2005-08-16 01:33:34Z bchoate $
...
sub rebuild_file {
    ...
    if ($needs_fileinfo) {
        ...
        if ((scalar @finfos == 1) && ($finfos[0]->file_path eq $file) && (($finfos[0]->file_url || '') eq $rel_url) && ($finfos[0]->template_id == $map->template_id)) {
    ...
    MT->run_callbacks('BuildFile', Context => $ctx,
                      ...
                      BuildResult => \$orig_html, Content => \$html,
    ...
    while (my $tmpl = $iter->()) {
        ...
        if ($needs_fileinfo) {
            ...
            my @finfos = MT::FileInfo->load({blog_id => $tmpl->blog_id,
                                             template_id => $tmpl->id});
            if ((scalar @finfos == 1) && ($finfos[0]->file_path eq $file) && (($finfos[0]->file_url || '') eq $rel_url)) {
                $finfo = $finfos[0];
            } else {
                foreach ( @finfos ) { $_->remove(); }
