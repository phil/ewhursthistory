...
#
# $Id: Context.pm 16059 2005-08-11 16:59:13Z bchoate $
...
sub add_tag {
    my $class = shift;
    my($name, $code) = @_;
    push @{$MT::Plugins{$MT::plugin_sig}{tags}}, "<\$MT${name}\$>" if $MT::plugin_sig;
...
sub post_process_handler {
    sub {
        ...
        if ($args) {
            ...
            # NEVER change the order of the blocks below. They'll be
            # applied in a different order and this will change the
            # result of some tags.
            ...
            if ($local_args{'dirify'}) {
                $str = dirify($str, $local_args{'dirify'});
            }
