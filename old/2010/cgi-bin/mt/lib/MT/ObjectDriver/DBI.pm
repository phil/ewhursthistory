...
#
# $Id: DBI.pm 15918 2005-08-09 17:11:01Z ezra $
...
sub load_iter {
    ...
    $sql = $tmp . $sql;
    my $dbh = $driver->{dbh};
    warn "load_iter - Preparing SQL: $sql" if $MT::DebugMode > 3;
...
sub load {
    ...
    $sql = $tmp . $sql;
    my $dbh = $driver->{dbh};
    warn "load - Preparing SQL: $sql" if $MT::DebugMode > 3;
    my $sth = $dbh->prepare($sql)
        or return $driver->error("Loading data failed with SQL error " . 
                                 $dbh->errstr);
    $sth->{private_is_tmp} = 1 if $tbl =~ m/^temp_/i;
...
sub count {
    ...
    my $dbh = $driver->{dbh};
    warn "count - Preparing SQL: $sql" if $MT::DebugMode > 3;
...
sub count_group_by {
    ...
    my $dbh = $driver->{dbh};
    warn "count_group_by - Preparing SQL: $sql" if $MT::DebugMode > 3;
...
sub exists {
    ...
    my $dbh = $driver->{dbh};
    warn "exists - Preparing SQL: $sql" if $MT::DebugMode > 3;
    my $sth = $dbh->prepare($sql) or return;
    $sth->execute($obj->id)
...
sub insert {
    ...
    my $dbh = $driver->{dbh};
    warn "insert - Preparing SQL: $sql" if $MT::DebugMode > 3;
    my $sth = $dbh->prepare($sql)
...
sub update {
    ...
    warn "update - Preparing SQL: $sql" if $MT::DebugMode > 3;
    my $sth = $dbh->prepare($sql)
        or return $driver->error($dbh->errstr);
    my $i = 1;
    my $col_defs = $obj->column_defs;
...
sub remove {
    ...
    my $dbh = $driver->{dbh};
    warn "remove - Preparing SQL: $sql" if $MT::DebugMode > 3;
    my $sth = $dbh->prepare($sql)
        or return $driver->error($dbh->errstr);
    $sth->execute($id)
...
sub remove_all {
    ...
    my $dbh = $driver->{dbh};
    warn "remove_all - Preparing SQL: $sql" if $MT::DebugMode > 3;
    my $sth = $dbh->prepare($sql)
        or return $driver->error($dbh->errstr);
    $sth->execute
