...
#
# $Id: App.pm 17330 2005-09-09 21:55:03Z bchoate $
...
sub handler ($$) {
    ...
    #undef $0; # $0 should be blank for handlers
...
sub init {
    ...
    MT->add_callback('TakeDown', 0, $app, \&cb_touch_blogs );
...
sub init_request {
    ...
    # @req_vars: membesr of the app object which are request-specific
...
sub cb_touch_blogs {
    my $blogs_touched = MT->instance->request('blogs_touched') || {};
...
sub login {
    ...
    if ($author) {
        ...
        if (!defined($author)) {
            ...
            $app->log($app->translate("Invalid login attempt from user '[_1]'", $user));
...
sub run {
    ...
    if (my $url = $app->{redirect}) {
        ...
        unless ($app->{no_print_body}) {
            ...
            $app->print($body);
            if ($MT::DebugMode) {
                $app->print("<pre>" . encode_html($app->{trace}) . "</pre>")
                    if $app->{trace} &&
...
sub process_mt_template {
    ...
    $body =~ s/<MT_ACTION\s+mode=(\w+)(?:\s+([^>]*))?>/
        ...
        $args ||= "";
        %args = $break->($args);
        $result .= $app->uri(mode => $mode, args => \%args);
    /ge;
    return $body; # . "<pre>[" . $result . "]</pre>";
}
...
sub build_page_in_mem {
    my $app = shift;
    my($tmpl, $param) = @_;
    $param->{needs_magic} = 1 if !exists($param->{needs_magic});
    $param->{magic_token} = $app->current_magic
        if $app->{author} && $param->{needs_magic};
    my $mode = $app->mode;
    $param->{"mode_$mode"} ||= 1;
    ...
    $tmpl->param(%$param);
    $app->l10n_filter($app->process_mt_template(($tmpl->output)));    
}
