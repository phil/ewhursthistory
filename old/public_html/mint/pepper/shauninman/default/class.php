...
class SI_Default extends Pepper
{
	...
	function onRecord() 
	{
		...
		$this->data['visits'] = $visits;
 		...
 		$res_title			= $this->escapeSQL(trim(str_replace('\n', ' ', preg_replace('/%u([\d\w]{4})/', '&#x$1;', htmlentities($_GET['resource_title'])))));
 		...
 		if (!empty($referer)) 
 		{
 			$referer_is_local	= (preg_match("/^([^:]+):\/\/([a-z09]+[\._-])*(".str_replace('.', '\.', implode('|', $this->Mint->domains)).")/i", $referer))?1:0;
...
<table>
	...
	<tr>
		<td><span><textarea id="ignoreReferringDomains" name="ignoreReferringDomains" rows="6" cols="30">{$this->prefs['ignoreReferringDomains']}</textarea></span></td>
	</tr>
</table>
<table class="snug">
	<tr>
...
HERE;
	...
	{	
		...
		$this->prefs['ignoreReferringDomains']	= $this->escapeSQL($_POST['ignoreReferringDomains']);
	}
	...
	function onCustom() 
	{
		...
		) 
		{
			...
			if (preg_match("/^https?:\/\/([a-z09]+[\._-])*($localDomains)/i", $resource)) 
			{
