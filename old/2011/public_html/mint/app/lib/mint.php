...
class Mint
{
	...
	var $feedback	= array
	(
		'button'	=> '', // empty is none, options are 'back', 'okay', 'done'
		'feedback'		=> '',
	...
	function Mint($args = array())
	{
		...
		if ($this->db['connected'])
		{
			if (!$this->_load())
			{
				...
				$this->cfg['installDir']	= preg_replace("/\/[^\/]*$/", '', ((isset($_SERVER['PHP_SELF']) && !empty($_SERVER['PHP_SELF']))?$_SERVER['PHP_SELF']:$_SERVER['SCRIPT_URL']));
				$this->cfg['installDomain']	= (!empty($_SERVER["HTTP_HOST"]) && $_SERVER["HTTP_HOST"] != $_SERVER['SERVER_NAME'])?$_SERVER["HTTP_HOST"]:$_SERVER['SERVER_NAME'];
	...
	/**************************************************************************
	 ...
	 Pulls the config data from the database and loads installed Pepper. Doesn't 
	 use $this->query() to avoid the standard SQL error when attempting to load
	 config data before Mint is fully installed.
	 **************************************************************************/
	function _load() 
	{
		// $this->logError('_load()');
	...
	/**************************************************************************
	 ...
	 "Rolls" our database and gives it a physical. _save() moved to record()
 	 **************************************************************************/
	function _tidyDB() 
	{
		// $this->logError('_tidyDB()');
		...
		// Hourly check-up
		if ($this->cfg['lastChecked'] < time() - (60 * 60)) 
		{
					...
					if ($rows)
					{
						...
						if ($totalHours)
						{
							...
							if ($this->getDatabaseSize() > ($this->cfg['preferences']['maxSize'] * 1024 * 1024))
							{
								...
								$query = "DELETE FROM `{$this->db['tblPrefix']}visit` LIMIT $removeRows";
								$this->query($query);
								$this->cfg['lastOptimized'] = 0;
								
								/** /
								$report  = 'Over already. <br />Query: '.$query.'<br />';
								$report .= 'Size difference: '.number_format(($sizeDiff / 1024 / 1024), 2).'<br />';
								$report .= 'Total rows to remove: '.$removeRows.'<br />';
							...
							{
								$query = "DELETE FROM  `{$this->db['tblPrefix']}visit` LIMIT $avgRowsHour";
								$this->query($query);
								$this->cfg['lastOptimized'] = 0;
								
								/** /
								$report  = 'Query: '.$query.'<br />';
								$report .= 'Visit table size: '.number_format(($size / 1024 / 1024), 2).'MB for '.$rows.' row(s)<br />';
								$report .= 'Total Hits/Hours: '.$totalHits.' / '.$totalHours.'<br />';
								$report .= 'Average row size: '.$avgRowSize.' ('.number_format(($avgRowSize / 1024 / 1024), 2).'MB)<br />';
								$report .= 'Average rows an hour: '.$avgRowsHour.'<br />';
								$report .= 'Average hourly growth: '.$avgGrowth.' ('.number_format(($avgGrowth / 1024 / 1024), 2).'MB)<br />';
								$report .= 'Need to trim? '.((($this->getDatabaseSize() + $avgGrowth) > ($this->cfg['preferences']['maxSize'] * 1024 * 1024))?'Yes':'No');
								$this->logError($report);
								/**/
							}
						}					
		...
		if ($this->cfg['lastOptimized'] < time() - (60 * 60 * 24 * 7)) 
		{
	...
	{
		// $this->logError('_verifyLicense()');
		
		if (preg_match('/^(127\.0\.0\.1|localhost)(:\d+)?$/i', $this->cfg['installTrim']))
		{
			return true;
		}
		
		$installDir		= urlencode("http://{$this->cfg['installDomain']}{$this->cfg['installDir']}");
		$activationKey	= urlencode($_POST['activation_key']);
...
HERE;
			...
			$this->logError('The Activation Key <strong>'.$_POST['activation_key'].'</strong> is not valid for: '.$this->cfg['installTrim']);
			return false;
		}
		else
		{
	...
	{
		// $this->logError('_checkForUpdates()');
		...
		{
			$html .= " <a href=\"{$this->cfg['update']['url']}\" class=\"update\"><span>Update Available</span></a>";
		}
	...
	function update() 
	{
			...
			{	
				...
				if ($result = $this->query($query))
				{
							...
							foreach($row as $oldColumn => $value)
							{
								switch($oldColumn)
								{
									...
									case 'referer':
										...
										if (!empty($value))
										{
											$save['referer_is_local'] = (preg_match("/^([^:]+):\/\/([a-z09]+[\._-])*(".str_replace('.', '\.', implode('|', $this->domains)).")/i", $value))?1:0;
										}
									case 'resource':
					...
					else
					{
						...
						// $this->query("DROP TABLE `pre120_{$this->db['tblPrefix']}_config`");
						// $this->query("DROP TABLE `pre120_{$this->db['tblPrefix']}visit`");
					}
				}
			}
			else
		...
		$this->logFeedback('<p>Mint has been updated successfully!</p>', 'okay', $this->cfg['installDir'].'/');
		$this->_save();
	}
	...
	{
		// $this->logError('getPathsToPepper()');
...
HERE;
	...
	function record() 
	{
		...
		if (!$this->db['connected'] || (isset($_COOKIE['MintIgnore']) && $_COOKIE['MintIgnore']=='true') || !$this->verifyKey()) { return; }
...
HERE;
	...
	function widget()
	{
		...
		if ($this->authenticate()) 
		{
			...
			$html = "<div id=\"for\">For {$this->cfg['siteDisplay']}</div><div id=\"links\"><a href=\"{$this->cfg['installFull']}\">Full Serve</a> | <a href=\"{$this->cfg['installFull']}\">View Site</a></div>$html";
		}
		else
		{
	...
	{
		// $this->logError('detectDocumentRoot()');
	...
	{
		// $this->logError('getPepperByClassName()');
	...
	{
		$html = "";
		// $offsets = array(-12, -11, -10, -9.5, -9, -8, -7, -6, -5, -4, -3.5, -3, -2, -1, 0, 1, 2, 3, 3.5, 4, 4.5, 5, 5.5, 5.75, 6, 6.5, 7, 8, 8.75, 9, 9.5, 10, 11, 12, 12.75, 13, 14);
	...
	function offsetMakeGMT($hours = false, $minutes = false, $seconds = false, $month = false, $day = false, $year = false)
	{
		...
		if (gettype($hours) == 'boolean')
		{
			$hours = $nowHours;
		}
		if (gettype($minutes) == 'boolean')
		{
			$minutes = $nowMinutes;
		}
		if (gettype($seconds) == 'boolean')
		{
			$seconds = $nowSeconds;
		}
		if (gettype($month) == 'boolean')
		{
			$month = $nowMonth;
		}
		if (gettype($day) == 'boolean')
		{
			$day = $nowDay;
		}
		if (gettype($year) == 'boolean')
		{
			$year = $nowYear;
		}
	...
	/**************************************************************************
	 ...
	 to return adjusted time. Missing the `c` and `r` formatting options but 
	 adds the `R` option to replace $Mint->formatDateTimeRelative()
	 **************************************************************************/
	...
	{	
		...
		(
			...
			$minutes,
			$seconds
		) = explode(' ', gmdate('j n Y G i s', $time));
		$offsetTime = mktime($hours + $offset, $minutes, $seconds, $month, $day, $year);
		$date = getdate($offsetTime);
		
		for ($i = 0; $i < strlen($format); $i++)
		{
			$c = $format{$i};
			
			if ( $c == '\\') // don't add escape slashes to output
			{
				continue;
			}
			else if ($i && $format{$i-1} == '\\') // directly output escaped characters
			{
				$formatted .= $c;
				continue;
			}
			...
			switch($c)
			{
				...
				case 'B': // Swatch Internet time
					$r = gmdate($c, $time); // Detached, Swatch time
					break;
				
				case 'c': // ISO 8601 date (added in PHP 5)
					$r = '*Missing Format Option for "'.$c.'"*'; // gmdate($c, $time);
					break;
				
				case 'd': // Day of the month, 2 digits with leading zeros
					$r = str_pad($date['mday'], 2, '0', STR_PAD_LEFT);
					break;
				
				case 'D': // A textual representation of a day, three letters
					$r = substr($date['weekday'], 0, 3);
					break;
				...
				case 'g': // 12-hour format of an hour without leading zeros	
					$r = ($date['hours'] <= 12)?(($date['hours'] == 0)?12:$date['hours']):$date['hours']-12;
					break;
				...
				case 'h': // 12-hour format of an hour with leading zeros	
					$r = str_pad((($date['hours'] <= 12)?(($date['hours'] == 0)?12:$date['hours']):$date['hours']-12), 2, '0', STR_PAD_LEFT);
					break;
				
				case 'H': // 24-hour format of an hour with leading zeros	
					$r = str_pad($date['hours'], 2, '0', STR_PAD_LEFT);
					break;
				
				case 'i': // Minutes with leading zeros	
					$r = str_pad($date['minutes'], 2, '0', STR_PAD_LEFT);
					break;
				
				case 'I': // Whether or not the date is in daylights savings time	
					$r = gmdate($c, $time); // Detached, could be wrong offset hours
					break;
				...
				case 'L': // Whether it's a leap year
					$r = (($date['year'] - 2000) % 4)?0:1;
					break;
				
				case 'm': // Numeric representation of a month, with leading zeros	
					$r = str_pad($date['mon'], 2, '0', STR_PAD_LEFT);;
					break;
				
				case 'M': // A short textual representation of a month, three letters	
					$r = substr($date['month'], 0, 3);
					break;
				...
				case 'O': // Difference to Greenwich time (GMT) in hours	
					$r = (($offset >= 0)?'+':'-').str_pad(abs($offset).'00', 4, '0', STR_PAD_LEFT);
					break;
				...
				case 'R': // Custom, relative to current time
					...
					if ($diff < 7) 
					{
						$r = ($diff > 1)?"$diff days ago":'Yesterday';
						break;
					}
					...
					if ($diff < 78) 
					{
						$r = ($diff > 1)?"$diff weeks ago":'Last Week';
						break;
					}
					
					$diff = round($diff / 52);
					$r = ($diff > 1)?"Around $diff years ago":'Last Year';
					
					break;
				
				case 's': // Seconds, with leading zeros
					$r = str_pad($date['seconds'], 2, '0', STR_PAD_LEFT);
					break;
				
				case 'S': // English ordinal suffix for the day of the month, 2 characters
					if ($date['mday'] != 11 || $date['mday'] != 12 || $date['mday'] != 13)
					{
						if (substr((string) $date['mday'], -1) == '1')
						{
							$r = 'st';
						}
						else if (substr((string) $date['mday'], -1) == '2')
						{
							$r = 'nd';
						}
						else if (substr((string) $date['mday'], -1) == '3')
						{
							$r = 'rd';
						}
					}
				...
				case 't': // Number of days in the given month
					$r = $this->offsetDate('j', $this->offsetMakeGMT(0, 0, 0, $date['mon'] + 1, 0, $date['year']));
					break;
				
				case 'T': // Timezone setting of this machine
					$r = date('T'); // Detached, offset doesn't change local machine's timezone, does not reflect returned time
					break;
	...
	/**************************************************************************
	 ...
	 eg. mins within the hour, hours within the day, days within the week, etc
	 **************************************************************************/
	function formatDateTimeRelative($time)
	{
		return $this->offsetDate('R', $time); //.' ('.$this->offsetDate('y.m.d g:ia O', $time).')';
	}
	...
	function formatDateRelative($time = null, $scale = "day", $i = 0) 
	{
		...
		return $relative; //.' ('.$this->offsetDate('y.m.d g:ia O', $time).')';
	}
	...
	function getFormattedVersion() 
	{
		...
		return '<abbr title="Mint (v'.str_pad($this->version,3,'0',STR_PAD_LEFT).')">Mint '.number_format($this->version/100,$len).'</abbr>';
	}
	...
	/**************************************************************************
	 ...
	 Trims a string to a specified length. Will trim from a space (for words), 
	 slash (for urls), or question mark or ampersand for query strings if at all 
	 possible. Can be anchored to the end of a string as well.
	 **************************************************************************/
		...
		if ($type == 'array' || $type == 'object')
		{
			...
			foreach($obj as $key=>$value)
			{
				...
				else
				{
					...
					if (preg_match('/(email|username|password|database)/i',$key))
					{
						$html .= "********\r";
					}
					else
